...
# Graceful OpenCV import - production containers may lack system libraries
try:
    import cv2
...
def train_model():
    ...
    if not OPENCV_AVAILABLE:
        logger.error("OpenCV not available for model training")
        return False
    try:
        faces = []
        labels = []
        if not FACE_DATA_DIR.exists():
            logger.warning("Face data directory does not exist")
            return False
...
def recognize_face(image_bytes, confidence_threshold=80):
    ...
    if not OPENCV_AVAILABLE:
        logger.error("OpenCV not available for face recognition")
        return None, None
    try:
